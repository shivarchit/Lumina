...
func TestFanoutAggregates(t *testing.T) {
	...
	res := (&Core{}).Fanout(targets, "setState", map[string]interface{}{"state": true})
...
func TestUpdateSavedIPsHealsByMAC(t *testing.T) {
	...
	c := &Core{cfg: config.Config{SavedDevices: []config.SavedDevice{
		{Name: "Desk", IP: "10.0.0.2", Mac: "aa:aa"},
		{Name: "Shelf", IP: "10.0.0.3", Mac: "bb:bb"},
	}}}
	if !c.UpdateSavedIPs(map[string]string{"aa:aa": "10.0.0.9"}) {
		t.Fatal("changed IP must report true")
	}
	if c.cfg.SavedDevices[0].IP != "10.0.0.9" || c.cfg.SavedDevices[1].IP != "10.0.0.3" {
		t.Fatalf("bad heal: %+v", c.cfg.SavedDevices)
	}
	if c.UpdateSavedIPs(map[string]string{"aa:aa": "10.0.0.9", "cc:cc": "10.0.0.1"}) {
...
func TestGroupTargetsResolvesAndFallsBack(t *testing.T) {
	...
	if got := c.GroupTargets("nope"); len(got) != 0 {
...
func TestSaveGroupRejectsEmptyName(t *testing.T) {
	c := &Core{}
	if err := c.SaveGroup(config.Group{Name: "  "}); err == nil {
...
func TestHintForLocalNetworkDenial(t *testing.T) {
	...
	if !Unreachable(err) {
		t.Fatal("EHOSTUNREACH must be unreachable-class")
	}
	if runtime.GOOS == "darwin" && HintFor(err) == "" {
		t.Fatal("expected a Local Network hint for EHOSTUNREACH on darwin")
	}
...
func TestSaveDeviceRequiresMAC(t *testing.T) {
	c := &Core{}
	if err := c.SaveDevice(config.SavedDevice{Name: "x", IP: "1.2.3.4"}); err == nil {
